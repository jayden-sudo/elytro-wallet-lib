...
export class EmailProof {
    ...
    > {
        ...
        await this._GenerateWitness.generateWitnessFile(wasmFile, circuitInputFile, witnessFile);
        ...
        try {
            if (this._rapidsnarkProverBin !== undefined) {
                ...
                await execPromise(`${this._rapidsnarkProverBin} ${zkeyFile} ${witnessFile} ${proofFile} ${publicFile}`);
                ...
                const _proof: unknown = JSON.parse(readFileSync(proofFile).toString('utf-8'));
                const _publicSignals: string[] = JSON.parse(readFileSync(publicFile).toString('utf-8'));
