...
export class ElytroWallet implements IElytroWallet {
    ...
    async getOnChainConfig(): Promise<Result<onChainConfig, Error>> {
        ...
        if (!_onChainConfig) {
            const _elytroWalletFactory = new ethers.Contract(this.elytroWalletFactoryAddress, ABI_ElytroFactory, this.provider);
            const elytroWalletLogic: string = await _elytroWalletFactory.getFunction("_WALLETIMPL").staticCall();
            const _elytroWallet = new ethers.Contract(elytroWalletLogic, ABI_Elytro, this.provider);
    ...
    async estimateUserOperationGas(validatorAddress: string, userOp: UserOperation, stateOverride?: Record<string, StateOverride>, signkeyType?: SignkeyType, semiValidGuardHookInputData?: GuardHookInputData): Promise<Result<UserOpGas, UserOpErrors>> {
        ...
        try {
            ...
            const userOpGasRet = await this.Bundler.eth_estimateUserOperationGas(_onChainConfig.OK.entryPoint, userOp, stateOverride);
